...
// src/handlers/createSolanaCollection.ts
import {
  composeContext,
...
var WalletSolana = class {
  ...
  umi;
  cluster;
  async fetchDigitalAsset(address) {
...
var supportedChainTuple = [..._SupportedChainList, "solana"];
...
// src/utils/deployEVMContract.ts
...
function compileWithImports(contractName, sourceCode) {
  const input = {
    language: "Solidity",
    sources: {
      [`${contractName}.sol`]: {
...
// src/contract/CustomERC721.sol
var CustomERC721_default = '// SPDX-License-Identifier: MIT\npragma solidity ^0.8.0;\n\nimport "@openzeppelin/contracts/token/ERC721/extensions/ERC721Enumerable.sol";\n\ncontract NFTContractName is ERC721Enumerable {\n    uint256 public maxSupply;\n    uint256 public currentTokenId;\n    address public owner;\n    uint256 public royalty;\n\n    constructor(\n        string memory _name,\n        string memory _symbol,\n        uint256 _maxSupply,\n        uint256 _royalty\n    ) ERC721(_name, _symbol) {\n        maxSupply = _maxSupply;\n        royalty = _royalty;\n        owner = msg.sender;\n    }\n\n    function mint(address _to) public {\n        require(currentTokenId < maxSupply, "Max supply reached");\n        currentTokenId++;\n        _mint(_to, currentTokenId);\n    }\n}\n';

// src/utils/deployEVMContract.ts
function generateERC721ContractCode(NFTContractName) {
  return CustomERC721_default.replace("NFTContractName", NFTContractName);
}
...
// src/actions/nftCollectionGeneration.ts
var _SupportedChainList2 = Object.keys(viemChains2);
var nftCollectionGeneration = {
  ...
  handler: async (runtime, message, _state, _options, callback) => {
    ...
    try {
      ...
      const supportedChains = [...chains, "solana", null];
      ...
      if ((content == null ? void 0 : content.chainName) === "solana") {
        ...
        elizaLogger3.log("Collection Info:", collectionAddressRes);
        if (callback) {
          callback({
...
Collection Link : ${collectionAddressRes.link}
 Address: ${collectionAddressRes.address}`,
        ...
        const rpcUrl = viemChains2[content.chainName].rpcUrls.default.http[0];
        const chain = viemChains2[content.chainName];
        ...
        elizaLogger3.log("ABI and Bytecode generated.");
...
async function createNFTMetadata({
  ...
  elizaLogger4.log("NFT Prompt:", nftPrompt);
  if (images.success && images.data && images.data.length > 0) {
    const image = images.data[0];
    const filename = `${tokenId}`;
    if (image.startsWith("http")) {
      elizaLogger4.log("Generating image url:", image);
    }
...
async function createNFT({
  ...
  if (nftInfo) {
    ...
    elizaLogger4.log("NFT ID:", nftAddressRes.address);
    return {
      network: "solana",
...
var _SupportedChainList3 = Object.keys(viemChains3);
...
var mintNFTAction = {
  ...
  handler: async (runtime, message, state, _options, callback) => {
    ...
    try {
      ...
      const content = res.object;
      elizaLogger5.log("Generate Object:", content);
      if (!isMintNFTContent(content)) {
        elizaLogger5.error("Invalid content for MINT_NFT action.");
        if (callback) {
          callback({
...
Collection Address: ${content.collectionAddress}
 ...
 NFT Link: ${nftRes.link}`,
        ...
        if (!privateKey) return null;
        const rpcUrl = viemChains3[content.chainName].rpcUrls.default.http[0];
        const chain = viemChains3[content.chainName];
        const provider = http2(rpcUrl);
...
var nftGenerationPlugin = {
  ...
  actions: [nftCollectionGeneration_default, mintNFTAction_default],
  evaluators: [],
  providers: []
};
export {
